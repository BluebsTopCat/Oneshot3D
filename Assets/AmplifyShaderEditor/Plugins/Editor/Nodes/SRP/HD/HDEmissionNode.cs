...
namespace AmplifyShaderEditor
{
    ...
    public sealed class HDEmissionNode : ParentNode
    {
        ...
        public const string EmissionHeaderLuminance =
            "ASEGetEmissionHDRColor{0}({1},{2},{3},GetInverseCurrentExposureMultiplier())";

        public const string EmissionHeaderEV100 =
            "ASEGetEmissionHDRColor{0}({1},ConvertEvToLuminance({2}),{3},GetInverseCurrentExposureMultiplier())";

        public const string IntensityUnityLabel = "Intensity Unit";

        public const string NormalizeColorLabel = "Normalize Color";

        [SerializeField] private HDEmissionIntensityUnit m_intensityUnit = HDEmissionIntensityUnit.Luminance;

        [SerializeField] private bool m_normalizeColor;
            ...
            bool ignoreLocalvar)
        {
            ...
            var colorValue = m_inputPorts[0].GeneratePortInstructions(ref dataCollector);
            var intensityValue = m_inputPorts[1].GeneratePortInstructions(ref dataCollector);
            var expositionWeightValue = m_inputPorts[2].GeneratePortInstructions(ref dataCollector);
            ...
            var varName = "hdEmission" + OutputId;
            var varValue = string.Empty;
            switch (m_intensityUnit)
            {
                default:
                case HDEmissionIntensityUnit.Luminance:
                    varValue = string.Format(EmissionHeaderLuminance, functionPrefix, colorValue, intensityValue,
                        expositionWeightValue);
                    break;
                case HDEmissionIntensityUnit.EV100:
                    varValue = string.Format(EmissionHeaderEV100, functionPrefix, colorValue, intensityValue,
                        expositionWeightValue);
                    break;
            }

            dataCollector.AddLocalVariable(UniqueId, CurrentPrecisionType, m_outputPorts[0].DataType, varName,
                varValue);
        ...
        public override void ReadFromString(ref string[] nodeParams)
        {
            ...
            Enum.TryParse(GetCurrentParam(ref nodeParams), out m_intensityUnit);
